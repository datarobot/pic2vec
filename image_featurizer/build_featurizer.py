...
def _decapitate_model(model, depth):
    ...
    ----------
    model: keras.model.Model
        The model being decapitated. Side-effect: model changed in place.
    depth: int
...
def _find_pooling_constant(features, num_pooled_features):
    ...
    -------
    int
        the integer pooling constant required to correctly splice the tensor layer for downsampling
    """
...
def _splice_layer(tensor, number_splices):
    ...
    list_of_spliced_layers:
        a list of the spliced sections of the original layer, with neighboring nodes
...
def build_featurizer(depth_of_featurizer, downsample, num_pooled_features=0,
    ...
    downsample : bool
        Boolean indicating whether to perform downsampling
    num_pooled_features : int
    ...
    loaded_model : keras.model.Model, optional
        If specified - use it for featurizing, istead of creating new one.
